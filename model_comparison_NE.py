...
for p in range(len(ppcs)):
    ...
    ax[1,1].grid(0.1)
    plt.suptitle(ppc_names[p]+" Priors (Netherlands)")
    plt.tight_layout()
    plt.savefig("./prior_predictive/NE_priors_"+ppc_names[p]+".png", dpi=600)
...
for p in range(len(ppcs)):
    ...
    ax[axes[p]].grid(0.2)
plt.suptitle("Posterior Predictives (Netherlands)")
plt.tight_layout()
plt.savefig("./posterior_predictive/NE_predictions.png", dpi=600)
