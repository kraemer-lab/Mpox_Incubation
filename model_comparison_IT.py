...
with pm.Model() as mod_l:
    u = pm.Gamma("u", mu=ps_l['mean'][0], sigma=ps_l['sd'][0])
    e = u * (tEndExposure - tStartExposure)
    a = pm.Gamma("a", mu=ps_l['mean'][1], sigma=ps_l['mean'][1])
    b = pm.Gamma("b", mu=ps_l['mean'][2], sigma=ps_l['mean'][2])
...
with pm.Model() as mod_g:
    u = pm.Gamma("u", mu=ps_g['mean'][0], sigma=ps_g['sd'][0])
    e = u * (tEndExposure - tStartExposure)
    a = pm.Gamma("a", mu=ps_g['mean'][1], sigma=ps_g['mean'][1])
    b = pm.Gamma("b", mu=ps_g['mean'][2], sigma=ps_g['mean'][2])
...
with pm.Model() as mod_w:
    u = pm.Gamma("u", mu=ps_w['mean'][0], sigma=ps_w['sd'][0])
    e = u * (tEndExposure - tStartExposure)
    a = pm.Gamma("a", mu=ps_w['mean'][1], sigma=ps_w['mean'][1])
    b = pm.Gamma("b", mu=ps_w['mean'][2], sigma=ps_w['mean'][2])
...
with pm.Model() as mod_wa:
    u = pm.Gamma("u", mu=ps_n['mean'][0], sigma=ps_n['sd'][0])
    e = u * (tEndExposure - tStartExposure)
    a = pm.Gamma("a", mu=ps_n['mean'][1], sigma=ps_n['mean'][1])
    b = pm.Gamma("b", mu=ps_n['mean'][2], sigma=ps_n['mean'][2])
...
plt.tight_layout()
plt.savefig('IT_model_comp_loo.png', dpi=600)
plt.show()
plt.close()
loo_df = pd.DataFrame(loo)
loo_df.to_csv("IT_model_comp_loo.csv")
...
plt.legend(prop={'size': 12})
plt.tight_layout()
plt.savefig('IT_model_comp_waic.png', dpi=600)
plt.show()
plt.close()
loo_df = pd.DataFrame(loo)
loo_df.to_csv("IT_model_comp_waic.csv")
...
table.scale(4.5, 4.5) 
plt.suptitle("Table 2. Estimated Mean Incubation Period (Italy)", size=30, y=1)
plt.tight_layout()
plt.savefig("IT_table2.png", dpi=600, bbox_inches="tight")
plt.show()
...
num_bins = x.shape[0]
counts, bin_edges = np.histogram(inc_day-inc_day.min(), bins=num_bins, density=True)
...
ax[1,1].plot(x.mean(axis=1), n_cdf_m, color="forestgreen", label="Negative Binomial CDF")
...
plt.tight_layout()
plt.savefig("IT_cdfs_plots.png", dpi=600)
plt.show()
plt.close()
...
posteriors.to_csv("IT_posteriors.csv")
