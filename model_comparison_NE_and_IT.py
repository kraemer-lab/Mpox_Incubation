...
df1 = pd.read_csv("./data/Netherlands_Mpox.csv")
df1 = df1.rename(columns={"Start date of exposure":"start",
                          "End date of exposure":"end",
                          "Symptom onset":"onset"})
df1 = df1.drop("Case ID", axis=1)
df2 = pd.read_csv("./data/Italy_Mpox.csv")
df2 = df2.drop(["index", "type"], axis=1)
...
for p in range(len(ppcs)):
    ...
    ax[1,1].grid(0.1)
    plt.suptitle(ppc_names[p]+" Priors (Netherlandas + Italy)")
    plt.tight_layout()
    plt.savefig("./prior_predictive/Both_priors_"+ppc_names[p]+".png", dpi=600)
...
plt.xlabel("ELPD LOO")
plt.title("LOO Model Comparison (Netherlandas + Italy)", size=12)
plt.grid(alpha=0.3)
plt.legend(prop={'size': 12})
plt.tight_layout()
plt.savefig('./model_comparison/Both_model_comp_loo.png', dpi=600)
plt.show()
plt.close()
loo_df = pd.DataFrame(loo)
loo_df.to_csv("./model_comparison/Both_model_comp_loo.csv")
...
az.plot_compare(loo, insample_dev=True, plot_kwargs={'color_insample_dev':'crimson', 'color_dse':'steelblue'})
plt.xlabel("Log")
plt.title("Waic Model Comparison (Netherlandas + Italy)", size=12)
plt.grid(alpha=0.3)
plt.legend(prop={'size': 12})
plt.tight_layout()
plt.savefig('./model_comparison/Both_model_comp_waic.png', dpi=600)
plt.show()
plt.close()
loo_df = pd.DataFrame(loo)
loo_df.to_csv("./model_comparison/Both_model_comp_waic.csv")
...
IT_means.to_csv("./posteriors/Both_means.csv")
...
table.scale(4.5, 4.5) 
plt.suptitle("Table 4. Estimated Mean Incubation Period (Netherlandas + Italy)", size=30, y=1)
plt.tight_layout()
plt.savefig("./posteriors/Both_table4.png", dpi=600, bbox_inches="tight")
plt.show()
...
plt.suptitle("Models CDFs (Netherlandas + Italy)")
plt.tight_layout()
plt.savefig("./posteriors/Both_cdfs_plots.png", dpi=600)
plt.show()
plt.close()
...
posteriors.to_csv("./posteriors/Both_posteriors.csv")
...
plt.suptitle("Netherlandas + Italy")
plt.tight_layout()
plt.savefig("./convergence_checks/Both_energy_plots.png", dpi=300)
plt.close()
...
for a in ax[:,1]:
    t = a.get_title()
    a.set_title(t, fontsize=22)
plt.suptitle("LogNormal (Netherlandas + Italy)", size=22)
plt.tight_layout()
plt.savefig("./convergence_checks/Both_lognormal_rankplot.png", dpi=300)
...
for a in ax[:,1]:
    t = a.get_title()
    a.set_title(t, fontsize=22)
plt.suptitle("Gamma (Netherlandas + Italy)", size=22)
plt.tight_layout()
plt.savefig("./convergence_checks/Both_gamma_rankplot.png", dpi=300)
...
for a in ax[:,1]:
    t = a.get_title()
    a.set_title(t, fontsize=22)
plt.suptitle("Weibull (Netherlandas + Italy)", size=22)
plt.tight_layout()
plt.savefig("./convergence_checks/Both_weibull_rankplot.png", dpi=300)
...
for a in ax[:,1]:
    t = a.get_title()
    a.set_title(t, fontsize=22)
plt.suptitle("NegativeBinomial (Netherlandas + Italy)", size=22)
plt.tight_layout()
plt.savefig("./convergence_checks/Both_negativebinom_rankplot.png", dpi=300)
...
for p in range(len(ppcs)):
    ...
    ax[axes[p]].grid(0.2)
plt.suptitle("Posterior Predictives (Netherlandas + Italy)")
plt.tight_layout()
plt.savefig("./posterior_predictive/Both_predictions.png", dpi=600)
